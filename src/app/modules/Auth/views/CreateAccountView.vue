    ...
    <v-form ref="formRef" @submit.prevent="submit" class="h-full">
      <v-card class="" width="400px">
        <v-card-item class="text-center">
          ...
          <h6 class="text-h6 pt-3 text-primary">Crear cuenta</h6>
        </v-card-item>
          ...
          <v-row>
            <v-col cols="12">
              ...
              <div class="text-caption text-center">
                Si eres estudiante, ingresa tu correo. Si está registrado en el
                sistema, podrás crear tu cuenta. Si no, contacta a
                administración.
              </div>
            </v-col>
            <v-col cols="12">
...
import { ref } from "vue";
import { _createAccount } from "@/app/modules/Auth/services";
...
const submit = async () => {
  ...
  if (!valid) return;
  loading.value = true;
  const response = await _createAccount(form.value);
  if (response) {
