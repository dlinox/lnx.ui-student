      ...
      <v-card :title="course?.name" subtitle="Registrar pago">
        <v-item-group mandatory class="border-t">
          <v-container style="height: auto; max-height: 280px; overflow-y: auto">
            <v-alert v-if="groupItems.length === 0" type="warning" variant="tonal">
              No hay grupos habilitados para este curso.
            </v-alert>
            <v-row dense>
              <v-col v-for="(item, index) in groupItems" :key="index" cols="12">
                <v-item v-slot="{ isSelected, toggle }">
                  <v-card :color="isSelected ? 'primary' : ''" :variant="isSelected ? 'tonal' : 'plain'" @click="
                      if (toggle) toggle();
                    ...
                    <v-scroll-y-transition>
                      <v-card-item>
                        ...
                        <br />
                        <small>Docente: <strong> {{ item.teacher }} </strong></small>
                        <br />
                        <small>
                          Laboratorio:
                        ...
                        <small>
                          Horarios:
                          <v-chip rounded="sm" class="rounded-0 me-1" density="compact" color="primary"
                            v-if="item.schedules">
                            {{ item.schedules?.days }} {{ item.schedules?.startHour }} -
                            {{ item.schedules?.endHour }}
                          </v-chip>
                          <v-chip rounded="sm" class="rounded-0 me-1" density="compact" color="primary" v-else>
                            No hay horarios
                          </v-chip>
                        </small>
                      </v-card-item>
        ...
        <v-card-item class="border-t border-b">
          <v-row>
            <v-col cols="6">
              <v-select v-model="form.paymentMethod" :items="[
                { title: 'BN - VENTANILLA', value: 7 },
                { title: 'BN - PAGALO.PE', value: 9 },
              ]" label="Pago en" :rules="ruleForm.paymentMethod"></v-select>
            </v-col>
            <v-col cols="6">
              <v-text-field v-model="form.paymentSequence" label="Numero de operacion" type="number"
                :rules="ruleForm.paymentSequence"></v-text-field>
            </v-col>

            <v-col cols="6">
              <v-text-field v-model="form.paymentAmount" readonly label="Monto" type="number"
                :rules="ruleForm.paymentAmount"></v-text-field>
            </v-col>

            <v-col cols="6">
              <v-text-field v-model="form.paymentDate" label="Fecha" type="date"
                :rules="ruleForm.paymentDate"></v-text-field>
            </v-col>
          </v-row>
        </v-card-item>
        <v-card-actions>
          ...
          <v-btn text="Inscribirme" color="primary" type="submit" variant="flat" value="submit"
